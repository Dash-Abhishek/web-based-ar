...
const TABLE_OBJ_URL = '../assets/model.obj';
const TABLE_MTL_URL = '../assets/materials.mtl'; 
let MODEL_SCALE = 0.4;
...
class App {
  ...
  async onSessionStarted(session) {
    ...
    DemoUtils.loadModel(TABLE_OBJ_URL, TABLE_MTL_URL).then(model => {
      this.model = model;
  ...
  onXRFrame(time, frame) {
    ...
    if (this.reticle.visible && !this.stabilized) {
      ...
      console.log("stabilized reticle");
  ...
  async onClick(e) {
    ...
    if (hits.length) {
      ...
      console.log("removing reticle");
      this.scene.remove(this.reticle);
    }
  }
/**
 * This method shrinks the model 
 * @author abhishek
 */
  shrink(){
    console.log("shrink fox");
    MODEL_SCALE= MODEL_SCALE-0.01;
    this.model.scale.set(MODEL_SCALE,MODEL_SCALE,MODEL_SCALE)
  }
/**
 * This method scales up the model 
 * @author abhishek
 */
  grow(){
    console.log("grow fox");
    MODEL_SCALE=MODEL_SCALE+0.01;
    this.model.scale.set(MODEL_SCALE,MODEL_SCALE,MODEL_SCALE);
  }
};
 ...
 * Usefull snippets
 */
// to find an object in a scene by name
// let fox =this.scene.children.find(c => c.name === 'fox');
// console.log(fox);


// raycast and get intersects.
